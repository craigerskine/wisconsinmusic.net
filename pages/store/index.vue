  ...
  <section>
    ...
    <ul class="mx-auto max-w-sm grid(& flow-row cols-1) gap-10 sm:(max-w-screen-lg grid-cols-2) lg:(grid-cols-3)">
      ...
      <li v-for="(item, index) in articlesStore">
        <a :href="item.link" class="group block rounded transition">
          <figure>
            <img :src="item.image || '/assets/img/_mast-placeholder.jpg'" :alt="item.title +' mast'" class="w-full h-auto object-cover rounded transition group-hover:(ring(4 blue-400)) group-focus:(ring(4 blue-400))" />
